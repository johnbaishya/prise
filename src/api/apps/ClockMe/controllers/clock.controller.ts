...
import ClockMeUserSiteAssigned from "../../../../apps/ClockMe/models/ClockMeUserSiteAssigned";
import Clock from "../../../../apps/ClockMe/models/Clock";
import { checkOwnership } from "../../../../libs/auth";
import { UpdateClockRequestBody } from "../../../../apps/ClockMe/types/reqBodyTypes";
import { clockStatus, ClockActionType } from "../../../../apps/ClockMe/types/enums";
...
export const checkUserHasRunningClock = async(req:UserRequest,res:Response)=>{
    try {
        ...
        let runningClock = await Clock.findOne({
            user_id,
            status:{$in:[clockStatus.active,clockStatus.inactive]}
        })
        if(runningClock){
...
export const updateClock = async(req:UserRequest,res:Response)=>{
    try {
        ...
        let axnEnm = ClockActionType;
        let action = body.action;
        let status:clockStatus = clockStatus.active;
        switch (action) {
            case axnEnm.clock_out:
                status = clockStatus.completed;
                break;
            case axnEnm.break_start:
                status = clockStatus.inactive;
                break
